<template>
  <v-container>
    <v-card class="mx-auto" max-width="800">
      <v-card-title>Réfraction subjective</v-card-title>
      <v-card-text>
...
const loadRefractionData = async (examId) => {
  ...
  try {
    ...
    alert("Impossible de charger les données de la réfraction subjective");
  } finally {
    loading.value = false;
  }
};
...
const submitForm = async () => {
  try {
    ...
    await dbService.saveSection(examId, "refraction-subjective", formData);
